    ...
    <div class="transfer" :style="{width,height}">
        <template v-if="mode === 'tree'">
            ...
            <div class="transfer-left">
                ...
                <div class="transfer-main">
                    ...
                    <!-- <slot name="from"></slot> -->
                    <el-tree ref='from-tree' :data="selfFromData" show-checkbox :node-key="node_key" @check='fromTreeChecked' :default-expanded-keys="from_expanded_keys" :props="defaultProps" :filter-node-method="filterNodeFrom" :default-expand-all="openAll" :render-content='renderContent' :default-checked-keys="defaultCheckedKeys">
                    </el-tree>
            ...
            <div class="transfer-right">
                ...
                <div class="transfer-main">
                    ...
                    <el-tree slot='to' ref='to-tree' :data="selfToData" show-checkbox :node-key="node_key" @check='toTreeChecked' :default-expanded-keys="to_expanded_keys" :props="defaultProps" :filter-node-method="filterNodeTo" :default-expand-all="openAll" :render-content='renderContent'>
                    </el-tree>
        ...
        <template v-else-if="mode === 'list'">
            ...
            <div class="transfer-center address-list-center">
                <p class="transfer-center-item">
                    <el-button type="primary" @click="addToList()" icon="el-icon-arrow-right" circle :disabled="from_disabled"></el-button>
                </p>
            </div>
            <div class="transfer-right">
                <div class="transfer-right-item" :class="{'transfer-right-small':false}">
                    ...
                    <div class="transfer-main">
                        ...
                        <ul class="address-list-ul">
                            <li class="address-list-li" v-for="item of toList" :key="item[node_key]">
                                <label>{{item[defaultProps.label]}}</label><label class="label-right">{{item.pLabel}}</label>
                                <i class="address-list-del" @click="clearList(item[node_key])">x</i>
                            </li>
                        </ul>
                    </div>
                </div>
...
export default {
    ...
    mounted () {
        if (this.from_check_keys.length > 0 && this.defaultTransfer) {
            this.$nextTick(() => {
                if (this.mode === 'tree') {
                    this.addToAims()
    ...
    methods: {
        ...
        fromAllBoxChange (val) {
            ...
            if (val) {
                ...
                this.from_check_keys = this.selfFromData
        ...
        toAllBoxChange (val) {
            ...
            if (val) {
                this.to_check_keys = this.selfToData
        ...
        clearList (id) {
            ...
            this.archiveList =
                id === 'all'
                    ? []
                    : this.archiveList.filter(item => item[this.node_key] !== id)
            this.toList =
                id === 'all'
                    ? []
                    : this.toList.filter(item => item[this.node_key] !== id)
            // 传递信息给父组件
    ...
    computed: {
        // 左侧数据
        selfFromData () {
            let fromArray = [...this.fromData, ...this.self_from]
        ...
        // 右侧数据
        selfToData () {
            let toArray = [...this.toData, ...this.self_to]
    ...
    watch: {
        ...
        filterList (newval, oldval) {
            if (oldval === '') {
                this.archiveList = cloneDeep(this.toList)
            }
            if (newval === '') {
                this.toList = cloneDeep(this.archiveList)
                this.archiveList = []
            }
