...
Vue.filter('toThousands', function(value, dec = false) {
    if (!value) {
        return 0
    }
    let val = (dec ? value.toFixed(0) : value) + ''
...
Vue.filter('toFixed', function(data, num = 1) {
    return data ? parseFloat(data).toFixed(num) : '0.0'
})
...
Vue.filter('dateFormat', function(dt, fmt = 'yyyy-MM-dd hh:mm:ss') {
    ...
    if (typeof dt === 'string') {
        ...
        dt = /^\d+$/.test(dt)
            ...
            : /.\+./.test(dt)
                ? new Date(dt)
                : dt.replace(/-/g, '/') // 传入的日期可能是个纯数字组成的字符串，如"1511107200000"
    }
...
Vue.filter('toPercent', function(data) {
    return data ? (parseFloat(data) * 100).toFixed(1) + '%' : '0.0%'
})
Vue.filter('toRate', function(data, total) {
    return data && total ? parseFloat((data / total) * 100).toFixed(1) : '0.0'
})
